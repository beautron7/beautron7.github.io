...
function chembalance(instr) {
  ...
  this.searchPtable=function(instr){
    ...
    var iterator = 0;
    try {
      var left_string = instr.match(/^(.*)=(.*)$/)[1]+"+";
      var right_string = instr.match(/^(.*)=(.*)$/)[2]+"+";
      var left_ions=left_string.match(/(.+?)\+/g);
      var right_ions=right_string.match(/(.+?)\+/g);
      var new_ions = [[],[]];
    } catch (e) {
      return "\n\n\nThere was an error with the input handling. To avoid this, use no spaces, put subscript after numbers, and make it look like this: H2O=H+O2"
    } finally {

    }
    for (var i = 0; i < left_ions.length; i++) {
      new_ions[0][i]=left_ions[i].match(/([A-Z][a-z]?)(\d)*/g)
    }
    for (var i = 0; i < right_ions.length; i++) {
      new_ions[1][i]=right_ions[i].match(/([A-Z][a-z]?)(\d)*/g)
    }
    //console.log(new_ions)
    /*var values = instr.match(/^(.*?)\+(.*?)\=(.*?)\+(.*?)$/);
    var molecules = [];

    molecules[1] = values[1];
    molecules[2] = values[2];
    molecules[3] = values[3];
    molecules[4] = values[4];
    var ions = [];
    for (var i = 1; (i <= 4)&&(iterator<maxiterations); i++) {
        iterator++;
        ions[i] = [];
        var remaining = molecules[i];
        ions[i].push(molecules[i].match(/^([A-Z][a-z]?\d*)?(.*)/)[1]);
        remaining = molecules[i].match(/^([A-Z][a-z]?\d*)?(.*)/)[2];
        while ((remaining !== "") && (ions[i][ions[i].length - 1] !== undefined)&&(iterator<maxiterations)) {
          iterator++;
          ions[i].push(remaining.match(/^([A-Z][a-z]?\d*)?(.*)/)[1]);
          remaining = remaining.match(/^([A-Z][a-z]?\d*)?(.*)/)[2];
        }
    }
    if(iterator>maxiterations){
      throw "increase the maxiterations value";
    }*/
    var listofelements=[];
    //var coefficents = [];
    for (var h = 0; h < 2; h++) {
      for (var i = 0; i <new_ions[h].length; i++) {
        new_ions[h][i].coefficents=1;
        for (var j = 0; j < new_ions[h][i].length;j++){
          var found = false;
          var element = this.searchPtable(new_ions[h][i][j].match(/[A-Z][a-z]?/)[0]);
          if(element===undefined){
            return "\n\n\nWe were unable to find the element: "+new_ions[h][i][j]+". "+(function(){if(j>0){return "If you meant to type '"+new_ions[h][i][j-1].short+new_ions[h][i][j][0].toLowerCase()+"', then make sure it is Capitalized and Spelled correctly. "}})()+"Fix it and the program should work."
          }
          if(new_ions[h][i][j].match(/\d+/)!==null){
            new_ions[h][i][j]={name:element.name,protons:element.number,short:element.small,subscript:+new_ions[h][i][j].match(/\d+/)[0]}
          } else {
            new_ions[h][i][j]={name:element.name,protons:element.number,short:element.small,subscript:1}
          }
          for (var k = 0; k < listofelements.length; k++) {
            if(new_ions[h][i][j].name == listofelements[k].name){
              found = true;
              listofelements[k].locations.push({H:h,I:i,J:j});
              listofelements[k].count++;
            }
          }
          if(!found){
            listofelements.push(JSON.parse(JSON.stringify(new_ions[h][i][j])));
            listofelements[listofelements.length-1].count=1;
            listofelements[listofelements.length-1].subscript=undefined;
            listofelements[listofelements.length-1].locations=[{H:h,I:i,J:j}];
          }
        }
      }
    }
    for (var k = 0; k < listofelements.length; k++) {
      //console.log("found:"+listofelements[k].name+" "+listofelements[k].count+"times");
    }
    /*for (var k = 0; k < listofelements.length; k++) {
      listofelements[k].balanced=false
      var element_occourences = 0;
      for (var h = 0; h < 2; h++) {
        for (var i = 0; i < ions[h].length; i++) {
          for (var j = 0; j < ions[i].length; j++) {
            if(new_ions[h][i][j].name==listofelements[k].name){
              console.log("found "+(listofelements[k].name)+" in "+i+","+j)
              listofelements[k].count++;
              j=1/0;
            }
          }
    ...
    while (!balanced) {
      ...
      for (var i = 0; i < listofelements.length; i++) {
        var rightside = 0;
        var leftside = 0;
        for (var j = 0; j < listofelements[i].locations.length; j++) {
          var subscript=new_ions[listofelements[i].locations[j].H][listofelements[i].locations[j].I][listofelements[i].locations[j].J].subscript;
          var coeff=new_ions[listofelements[i].locations[j].H][listofelements[i].locations[j].I].coefficents;
          if(listofelements[i].locations[j].H==0){
            leftside+=subscript*coeff;
          }
          if(listofelements[i].locations[j].H==1){
            rightside+=subscript*coeff;
          }
        }
        //console.log(listofelements[i].name+": "+rightside+" on the right, and "+leftside+"on the left");
      ...
      if(!balanced){
        var h=0,i=0
        new_ions[h][i].coefficents++;
        while(new_ions[h][i].coefficents>cutoff){
          new_ions[h][i].coefficents=1;
          if(i+1>=new_ions[h].length){
            if(h+1>=new_ions.length){
              console.log("\n\n\nThat equasion either cannot be balanced, or it would need a coefficent larger than "+cutoff);
              return {line:154,error:"no combination works"};
              balanced=true;
              cutoff/=0;
            } else{
              i=0;
          ...
          if(new_ions[h][i].coefficents==undefined){
            new_ions[h][i].coefficents=1;
          }
          new_ions[h][i].coefficents++;
        }
        //console.log("h:"+h+" i:"+i+" coeff:"+new_ions[h][i].coefficents)
        while(new_ions[h][i].coefficents==undefined){
          new_ions[h][i].coefficents=1;
          //console.log(new_ions[h][i].coefficents)
        }
      }
    }
    /*for (var a = 1; a < 10; a++) {
      console.log(".\b")
      ions[1].coefficents=a
      for (var b = 1; b < 10; b++) {
        ions[2].coefficents=b
        for (var c = 1; c < 10; c++) {
          ions[3].coefficents=c
          for (var d = 1; d < 10; d++) {
            ions[4].coefficents=d;
            balanced=true;
            for (var i = 0; i < listofelements.length; i++) {
              var rightside = 0;
              var leftside = 0;
              for (var j = 0; j < listofelements[i].locations.length; j++) {
                var subscript=ions[listofelements[i].locations[j].ionI][listofelements[i].locations[j].ionJ].subscript
                if(listofelements[i].locations[j].ionI==1){
                  leftside+=a*subscript;
                }
                if(listofelements[i].locations[j].ionI==2){
                  leftside+=b*subscript;
                }
                if(listofelements[i].locations[j].ionI==3){
                  rightside+=c*subscript;
                }
                if(listofelements[i].locations[j].ionI==4){
                  rightside+=d*subscript;
                }
              }
              //console.log(listofelements[i].name+": "+rightside+" on the right, and "+leftside+"on the left");
              if(rightside!=leftside){
    ...
    var resultstring = "";
    for (var h = 0; h < new_ions.length; h++) {
      if(h!=0){resultstring+="= "}
      for (var i = 0; i < new_ions[h].length; i++) {
        resultstring+=new_ions[h][i].coefficents+"*";
        for (var j = 0; j < new_ions[h][i].length-1; j++) {
          resultstring+=new_ions[h][i][j].short;
          if(new_ions[h][i][j].subscript!=1){
            resultstring+=new_ions[h][i][j].subscript;
          }
        }
        resultstring+=new_ions[h][i][new_ions[h][i].length-1].short;
        if(new_ions[h][i][j].subscript!=1){
          resultstring+=new_ions[h][i][j].subscript;
        }
        resultstring+=" ";
    ...
    /*if((new_ions[1].coefficents==null)||(new_ions[1].coefficents==undefined)){
    } else {
      resultstring+=new_ions[1].coefficents+"*"
    }
    for (var element in new_ions[1]) {
      if(new_ions[1][element]["short"]==undefined){

      } else {
        resultstring+=new_ions[1][element].short;
        if(new_ions[1][element].subscript!=1){
          resultstring+=new_ions[1][element].subscript;
        }
      }
    }*/
...
//chembalance("H8+O6=H2O");
//chembalance("H2O=H2+O")
if (typeof window === 'undefined') {
  for (var i = 0; i < 100; i++) {
    console.log("")
  }
  console.log("We detected you are in Node")
  process.argv.forEach(function (val, index) {
    if(index==2){
      if (typeof val=="string") {
        console.log(chembalance(val))
      }
    }
    if(index>100){
      console.log("Is this how you spend your time?")
    } else if(index>2){
      console.log("Please do not have excess spaces")
    }
  });
} else {
  console.log("We detected you are in a browser. You should now have a chembalance function. try chembalance('HCl+SO4=H2O+S+Cl')")
}

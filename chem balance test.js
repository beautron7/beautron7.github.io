...
function chembalance(instr) {
    var cutoff = 20;
    if (instr == "") {
        return {message:"Please enter an equasion",success:false};
    }
    ...
    try {
        ...
        return {message:"\n\n\nThere was an error with the input handling. To avoid this, use no spaces, put subscript after numbers, and make it look like this: H2O=H+O2",success:false}
    }
        ...
        for (var i = 0; i < new_ions[h].length; i++) {
            ...
            for (var j = 0; j < new_ions[h][i].length; j++) {
                ...
                if (element === undefined) {
                    return {message:"\n\n\nWe were unable to find the element: " + new_ions[h][i][j] + ". " + (function() {
                          if (j > 0) {
                              return "If you meant to type '" + new_ions[h][i][j - 1].short + new_ions[h][i][j][0].toLowerCase() + "', then make sure it is Capitalized and Spelled correctly. "
                          }
                      })() + "Fix it and the program should work."
                    ,success:false}
                }
                if (new_ions[h][i][j].match(/\d+/) !== null) {
                    new_ions[h][i][j] = {
                        ...
                        short: element.small,
                        subscript: +new_ions[h][i][j].match(/\d+/)[0]
                    }
                } else {
                    new_ions[h][i][j] = {
                        name: element.name,
                ...
                for (var k = 0; k < listofelements.length; k++) {
                    if (new_ions[h][i][j].name == listofelements[k].name) {
                        found = true;
                        listofelements[k].locations.push({
                            H: h,
                            I: i,
                            J: j
                        });
                ...
                if (!found) {
                    ...
                    listofelements[listofelements.length - 1].locations = [{
                        H: h,
                        I: i,
                        J: j
                    }];
            ...
            while (new_ions[h][i].coefficents > cutoff) {
                ...
                if (i + 1 >= new_ions[h].length) {
                    if (h + 1 >= new_ions.length) {
                        return {message:"\n\n\nThat equasion either cannot be balanced, or it would need a coefficent larger than " + cutoff,success:false}
                        balanced = true;
                        cutoff /= 0;
                    } else {
                        i = 0;
    ...
    return {message:resultstring,success:true}
}

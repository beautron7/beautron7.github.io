  ...
  this.searchPtable=function(instr){
    ...
    for (var i = 1; i <=4; i++) {
      for (var j = 0; j < ions[i].length;j++){
        var found = false;
        var element = this.searchPtable(ions[i][j].match(/[A-Z][a-z]?/)[0])
        if(ions[i][j].match(/\d+/)!==null){
          ions[i][j]={name:element.name,protons:element.number,short:element.small,subscript:+ions[i][j].match(/\d+/)[0]}
        } else {
          ions[i][j]={name:element.name,protons:element.number,short:element.small,subscript:1}
        }
        for (var k = 0; k < listofelements.length; k++) {
          if(ions[i][j].name == listofelements[k].name){
            found = true;
            listofelements[k].locations.push({ionI:i,ionJ:j});
            listofelements[k].count++;
          }
        }
        if(!found){
          listofelements.push(JSON.parse(JSON.stringify(ions[i][j])));
          listofelements[listofelements.length-1].count=1;
          listofelements[listofelements.length-1].subscript=undefined;
          listofelements[listofelements.length-1].locations=[{ionI:i,ionJ:j}];
        }
      }
    }
    for (var h = 0; h < listofelements.length; h++) {
      listofelements[h].balanced=false
      var element_occourences = 0;
      for (var i = 1; i < ions.length; i++) {
        //console.log(i);
        for (var j = 0; j < ions[i].length; j++) {
          if(ions[i][j].name==listofelements[h].name){
            console.log("found "+(listofelements[h].name)+" in "+i+","+j)
            listofelements[h].count++;
            j=1/0;
          }
        }
      }
    ...
    var resultstring = "";
    if((ions[1].coefficents==null)||(ions[1].coefficents==undefined)){
    } else {
      resultstring+=ions[1].coefficents+"*"
    }
    for (var element in ions[1]) {
      if(ions[1][element]["short"]==undefined){

      } else {
        resultstring+=ions[1][element].short;
        if(ions[1][element].subscript!=1){
          resultstring+=ions[1][element].subscript;
        }
      }
    }
    if((ions[2].coefficents==null)||(ions[2].coefficents==undefined)){
      resultstring+=" + "
    } else {
      resultstring+=" + "+ions[2].coefficents+"*"
    }
    for (var element in ions[2]) {
      if(ions[2][element]["short"]==undefined){

      } else {
        resultstring+=ions[2][element].short;
        if(ions[2][element].subscript!=1){
          resultstring+=ions[2][element].subscript;
        }
      }
    }
    if((ions[3].coefficents==null)||(ions[3].coefficents==undefined)){
      resultstring+=" = "
    } else {
      resultstring+=" = "+ions[3].coefficents+"*"
    }
    for (var element in ions[3]) {
      if(ions[3][element]["short"]==undefined){

      } else {
        resultstring+=ions[3][element].short;
        if(ions[3][element].subscript!=1){
          resultstring+=ions[3][element].subscript;
        }
      }
    }
    if((ions[4].coefficents==null)||(ions[4].coefficents==undefined)){
      resultstring+=" + "
    } else {
      resultstring+=" + "+ions[4].coefficents+"*"
    }
    for (var element in ions[4]) {
      if(ions[4][element]["short"]==undefined){

      } else {
        resultstring+=ions[4][element].short;
        if(ions[4][element].subscript!=1){
          resultstring+=ions[4][element].subscript;
        }
      }
    }
